-- ============================================================================
-- Migration 0013: Free-tier cap 150 -> 100
-- The cap is enforced in the DB (these SECURITY DEFINER RPCs are the real gate,
-- not the UI), so lowering it means re-defining the three functions from 0008
-- with 100 in place of 150. Bodies are otherwise identical to 0008.
-- ============================================================================
...
begin
  ...
  return jsonb_build_object(
    ...
    'session', case when v_session.id is null then null else jsonb_build_object(
      ...
      'is_full', v_session.total_applications_received >= 100
